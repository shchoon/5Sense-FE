...
export default function DaySchedule({ dateData }: IProps) {
      ...
      class: [
        {
          ...
          teacherName: '조성훈',
          roomNum: '1호실',
          className: '체형 교정 및 이완을 위한 삶의 균형 찾기/ 스트레스로부터 벗어 나기'
        },
        {
          classType: 'time',
          hour: '9',
  ...
  useEffect(() => {
    ...
    let currentTimeClassIndex = classData.findIndex(data => data.time.split(':')[0] == String(currentHour))
                ...
                <div className="flex w-[77px] h-6 gap-5 items-center">
                  <div
                    className={`w-[51px] text-right ${
                      currentHour === Number(data.time.split(':')[0]) ? 'text-indigo-500' : 'text-black'
